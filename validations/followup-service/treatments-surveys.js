...
const addTreatmentSurvey = (treatmentSurveyData) => {
    ...
    if(callDuration && typeof callDuration != 'number') return { isAccepted: false, message: 'Call duration format is invalid', field: 'callDuration' }

    if(typeof improvement != 'number' || improvement > 5 || improvement < 0) return { isAccepted: false, message: 'Improvement format is invalid', field: 'improvement' }

    if(typeof isOverallHealthChanged != 'boolean') return { isAccepted: false, message: 'Is overall health changed format is invalid', field: 'isOverallHealthChanged' }

    if(typeof isExperiencedSideEffects != 'boolean') return { isAccepted: false, message: 'Is experienced side Effects format is invalid', field: 'isExperiencedSideEffects' }
    ...
    return { isAccepted: true, message: 'data is valid', data: treatmentSurveyData }
}

const updateTreatmentSurvey = (treatmentSurveyData) => {
...
module.exports = { addTreatmentSurvey, updateTreatmentSurvey }